...
const Kaprekar = n => {
    const kpkr = (n, r=[]) => {
      if (n == 6174) return r
      let [k, s] = ['', [...(n+'').padEnd(4,0)].sort()],
          [x, y] = [s.join(''), s.reverse().join('')]
      k = [...(+y - +x +'').padStart(4,0)].join('')
      return kpkr(+k, [...r, [y, x, k]])
    }
    let [i,r,c] = [0, [],['\n---------- The Mysterious Number 6174 ----------']]
    if (n > 1000 && new Set(n+'').size == 1) {
      c = [...c, '\nError, n cannot be a repdigit.']
    } else {
      r = kpkr(n)
      c = [...c, `\nNumber of iterations: ${r.length}`, '\nIterations:\n']
      for (let [a, b, k] of r) c = [...c, `Iteration Nr. ${++i}: ${a} - ${b} = ${k}`]
    }
    return [...c, '\n------------------------------------------------'].join('\n')
  }
